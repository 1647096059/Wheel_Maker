...
1. 复核载荷转移符号。
2. 明确 `ax / ay` 的物理定义。
3. 将 `delta` 和 `contact_half_len` 移入 `Swerve_Dynamics_Core` debug 输出。
4. 增加静态、前向加速、横向加速、纯旋转单元测试。
...
```text
四轮舵轮底盘早期选型与参数敏感性分析工具。
当前模型为简化准静态估算模型，适合用于方案比较和工程直觉分析，不作为最终实车安全裕度认证依据。
```
