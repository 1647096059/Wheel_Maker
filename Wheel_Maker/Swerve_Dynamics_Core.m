...
%   debug        - 包含微观物理过程的详细结构体，供上层可视化调用
    ...
    validatePositiveParam(p.swerve_rubber_thickness, 'swerve_rubber_thickness');
    ...
    % 单位转换：长度参数从 mm 转换为 m，供 SI 物理公式使用
    wheel_base_x_m    = p.swerve_wheel_base_x    / 1000;
    wheel_base_y_m    = p.swerve_wheel_base_y    / 1000;
    wheel_radius_m    = p.swerve_wheel_radius     / 1000;
    rubber_thickness_m = p.swerve_rubber_thickness / 1000;
    wheel_width_m     = p.swerve_wheel_width      / 1000;
    h_cog_m           = p.swerve_h_cog            / 1000;

    if rubber_thickness_m >= wheel_radius_m
        error('Swerve_Dynamics_Core:ParamError', 'swerve_rubber_thickness 必须小于 swerve_wheel_radius。');
    end
    ...
    m = p.swerve_m_total;
    g = p.g;
    Lx_full = wheel_base_x_m;
    Ly_full = wheel_base_y_m;
    h       = h_cog_m;
    ...
    Fx_total = m * ax;
    Fy_total = m * ay;
    R_outer = wheel_radius_m;
    t_rubber = rubber_thickness_m;
    R_hub = R_outer - t_rubber;
    w = wheel_width_m;
    ...
    Bottoming_Warning = zeros(1, 4);  % 新增：包胶压缩触底警告
    Fx_i_all = zeros(1, 4);
    Fy_i_all = zeros(1, 4);
    ...
    for i = 1:4
        ...
        delta_raw = Fz(i) / (E * w);
        delta = min(delta_raw, t_rubber);
        
        % 检测包胶是否完全压缩（触底）
        if delta_raw >= t_rubber * 0.95  % 95% 阈值，留有小余量
            Bottoming_Warning(i) = 1;
        end
        
        contact_expr = max(0, 2 * R_outer * delta - delta^2);
        ...
        T_roll = Fz(i) * R_outer * p.swerve_roll_resistance * p.swerve_carpet_factor;

        % 驱动总力矩 = 滚阻 + 静态机械损耗 + 惯性推力(经由滑移效率放大)
        T_drv_wheel = T_roll + ...
            p.swerve_T_mech_drive + ...
            (F_total_i * R_outer) / p.swerve_eta_slip;
    ...
    debug.mu = mu;
    debug.E = E;
    debug.R_outer = R_outer;
    debug.R_hub = R_hub;
    debug.rubber_thickness = t_rubber;
    ...
    debug.bottoming_warning = Bottoming_Warning;
end
